...
export default function Checkout() {
  ...
  const validation = Yup.object().shape({
    details: Yup.string()
      ...
      .min(5, "Details must be more than 5 Characters")
      .max(100, "Details must be less than 100 Characters"),
    phone: Yup.string()
      .required("Phone is required")
      .matches(phoneRegex, "Invalid phone number *Enter egyptian number*"),
  ...
  let { handleSubmit, handleChange, handleBlur, values, errors, touched } =
    useFormik({
      ...
      onSubmit: () =>
        handleCheckout(
          cart?.cartId,
          "https://fresh-cart-ecommerce-five.vercel.app/",
          values
        ),
    });
        ...
        <form onSubmit={handleSubmit}>
          <div className="relative z-0 w-full mb-5 group">
            <input
              ...
              name="details"
              id="details"
              className=" focus:outline-0 block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-green-600 peer"
              placeholder=" "
              required
            />
            <label
